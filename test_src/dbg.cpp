...
arr_element *test_space, *tmp_space, *load_space, *std_space;
arr_element *spaces[4] = {test_space, load_space, tmp_space, std_space};
...
int main(int argc, char* argv[])
{
    if (argc == 1) {
        cout <<
            "sort-test debug version\n"
            "\n"
            "usage: \n"
            "    dbg [int_testset_path] [option] [test sorts list]\n"
            "    eg. dbg ./Datatest_Test std_sort\n"
            "\n"
            "int_testset_path:\n"
            ...
            "\n"
            "use dbg --help [sort's name or command] for more information\n";
        return 0;
    }
    ...
    std_space = new arr_element[require_space_size];

    STDSort noerror_sort(std_space, require_space_size);
    ...
    for(int i = 1; i <= test_sets.size(); i++)
    {
        ...
        noerror_sort.primitive(load_space + 1, load_space[0]);
        test_sorts(load_space + 1, test_count, load_space[0]);

        cout << "test point " << i << " passed" << endl;
    }
