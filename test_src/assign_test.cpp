...
)
{
    for (int i = 0; i < testing_sorts_count; i++) {
        ...
        testing_sorts[i]->block(swap_count);
        testing_sorts[i]->block("swap_times");
        testing_sorts[i]->block(assign_count);
        testing_sorts[i]->block("assign_times");
...
int main(int argc, char* argv[])
{
    if (argc == 1) {
        cout <<
            "sort-test assign_test version\n"
            "\n"
            "usage: \n"
            "    assign_test [int_testset_path] [option] [test sorts list]\n"
            "    eg. assign_test ./Datatest_Test std_sort\n"
            "\n"
            "int_testset_path:\n"
            ...
            "\n"
            "use assign_test --help [sort's name or command] for more information\n";
        return 0;
    }
